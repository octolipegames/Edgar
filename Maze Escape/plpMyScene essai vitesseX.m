...
//        [Edgar takeDamage];
    ...
    moveRight = false;
    vitesseX = 0;
    Edgar.physicsBody.velocity = CGVectorMake(0, 0);
...
//            [self EdgarDies];
        ...
        if([contactNode isKindOfClass:[plpPlatform class]])
        {
            NSLog(@"Plateforme");
            stopRequested = TRUE;
            contextVelocityX = [(plpPlatform *)contactNode getVelocityX];
        }
...
-(void)update:(CFTimeInterval)currentTime {
    ...
    float vitesseXEdgar = Edgar.physicsBody.velocity.dx; // attention: s'il est sur plateforme on la conserve
...
//        [Edgar runAction: [SKAction repeatAction:stoppe count:1]];
    ...
    if (moveRightRequested == true){// && vitesseXEdgar < 0.5){
        ...
        if(moveRight != true){
            ...
            [Edgar removeAllActions];
            [Edgar walkingEdgar];
//            vitesseX = 150 + contextVelocityX;
            Edgar.physicsBody.velocity=CGVectorMake(vitesseXEdgar+150, Edgar.physicsBody.velocity.dy);
