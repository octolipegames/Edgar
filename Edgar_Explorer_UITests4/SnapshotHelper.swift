...
public class Snapshot: NSObject {
    ...
    class func setLaunchArguments(app: XCUIApplication) {
        ...
        do {
            ...
            let matches = regex.matches(in: launchArguments, options: [], range: NSRange(location:0, length:launchArguments.String.count))
    ...
    class func waitForLoadingIndicatorToDisappear() {
        let query = XCUIApplication().statusBars.children(matching: .other).element(boundBy: 1).childrenMatchingType(.Other)
